...
public class BuildExecutor {
		...
		if (!build_completed()) execute_build();
			...
			if (GameInfoCache.count(Units.ZERG_DRONE) > 45 || GameInfoCache.count(Units.ZERG_EXTRACTOR) == 0) {
				if ((Game.gas() < 400 && GameInfoCache.in_progress(Units.ZERG_EXTRACTOR) == 0) || Game.gas() < 150) {
					if ((BaseManager.active_extractors() + GameInfoCache.in_progress(Units.ZERG_EXTRACTOR) < Build.ideal_gases) && ((BaseManager.active_extractors() + GameInfoCache.in_progress(Units.ZERG_EXTRACTOR)) < ((GameInfoCache.count(Units.ZERG_DRONE) - 15) / 8))) {
	...
	public static UnitType next_army_unit() {
		...
		for (UnitType u: Composition.comp()) {
			...
			if (u == Units.ZERG_CORRUPTOR && GameInfoCache.count(Units.ZERG_CORRUPTOR) >= 20) continue;
			...
			if (u == Units.ZERG_CORRUPTOR && GameInfoCache.count(Units.ZERG_CORRUPTOR) < 5 && Game.army_supply() > 30 && BaseManager.active_extractors() >= 4 && GameInfoCache.count_friendly(Units.ZERG_SPIRE) > 0) return Units.ZERG_CORRUPTOR;
