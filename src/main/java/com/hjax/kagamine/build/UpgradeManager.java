...
public class UpgradeManager {
	...
	static {
		...
		upgrades.put(Units.ZERG_MUTALISK, Arrays.asList(Upgrades.BURROW, Upgrades.ZERG_FLYER_WEAPONS_LEVEL1, Upgrades.ZERG_FLYER_WEAPONS_LEVEL2, Upgrades.ZERG_FLYER_WEAPONS_LEVEL3, Upgrades.ZERG_FLYER_ARMORS_LEVEL1, Upgrades.ZERG_FLYER_ARMORS_LEVEL2, Upgrades.ZERG_FLYER_ARMORS_LEVEL3));
			...
			outer: for (Upgrade u : upgrades.getOrDefault(ut, Arrays.asList())) {
				...
				if (!(Game.has_upgrade(u)) && !GameInfoCache.is_researching(u)) {
					...
					for (UnitType t: upgraders.get(u)) {
						if ((t.equals(Units.ZERG_EVOLUTION_CHAMBER) && GameInfoCache.count(t) < 1 && (GameInfoCache.count(Units.ZERG_DRONE) > 25) && GameInfoCache.get_opponent_race() == Race.ZERG) || (t.equals(Units.ZERG_EVOLUTION_CHAMBER) && GameInfoCache.count(t) < 2 && (GameInfoCache.count(Units.ZERG_DRONE) > 65))) {
