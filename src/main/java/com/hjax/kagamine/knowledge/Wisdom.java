...
public class Wisdom {
	...
	public static boolean should_build_drones() {
		...
		if (all_in_detected() && EnemyModel.enemyWorkers() + 12 < BuildExecutor.count(Units.ZERG_DRONE)) {
			return false;
		}
		return (BuildExecutor.count(Units.ZERG_DRONE) < BuildExecutor.worker_cap());
	}
